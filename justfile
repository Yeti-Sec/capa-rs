...
# Requires: pip install ziglang && cargo install cargo-zigbuild
# ============================================================================

linux-target := "x86_64-unknown-linux-gnu"
zig-path := "C:\\Users\\yeti-sec\\miniconda3\\Lib\\site-packages\\ziglang"

# Build capa-rs for Linux in release mode
build-linux:
    $env:PATH = "{{zig-path}};$env:PATH"; cargo zigbuild --release --target {{linux-target}} --bin capa-rs

# Build capa-rs for Linux with .NET support
build-linux-dotnet:
    $env:PATH = "{{zig-path}};$env:PATH"; cargo zigbuild --release --target {{linux-target}} --bin capa-rs --features dotnet
