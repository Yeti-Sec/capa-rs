...
#### Linux: IDA shared library linking

On Linux, IDA symbols are resolved at link time from the actual shared libraries (`libida.so`, `libidalib.so`) rather than from stub `.lib` files like on Windows. The `capa-cli/build.rs` and `capa-backend/build.rs` emit `rustc-link-search` and `rustc-link-lib=dylib` directives pointing at `IDADIR` (defaults to `/opt/ida`).

If you see linker errors like `undefined reference to 'open_database'` on Linux, ensure `IDADIR` points to your IDA installation directory containing the `.so` files:

```bash
export IDADIR="$HOME/ida-pro-9.2"
```

#### Linux: autocxx FILE type wrappers (idalib-sys)

The IDA SDK's `gdl_graph_t` has virtual methods that take `FILE*` parameters. On Linux, glibc's `FILE` is `struct _IO_FILE` which is non-POD (has constructors, non-trivial members). autocxx tries to generate `alloc`/`copy`/`move` wrappers for it, which fails to compile under clang/gcc.

The vendored `idalib-sys/src/lib.rs` includes `block!("_IO_FILE")`, `block!("__sFILE")`, and `block!("_iobuf")` in the `include_cpp!` macro to prevent this. If you update idalib-sys from upstream and lose these directives, the Linux build will fail with errors in `autocxxgen_ffi.h` about non-copyable types.

#### Linux: `ea_t` vs `uint64_t` in C++ FFI headers

IDA's `ea_t` is defined as `unsigned long long` on all platforms (via the SDK's `llong.hpp`). However, `uint64_t` is `unsigned long` on Linux (LP64 model) but `unsigned long long` on Windows (LLP64 model). The CXX bridge maps `c_ulonglong` to `unsigned long long` specifically.

Any custom C++ FFI header (like `ua_extras.h`, `nalt_extras.h`) must use `ea_t` — not `uint64_t` — in function signatures. Using `uint64_t` compiles on Windows but causes type mismatches on Linux where the CXX bridge expects `unsigned long long` but gets `unsigned long`.

#### Linux: `cargo_warnings(false)` hides compile errors

The `idalib-sys/build.rs` previously called `cargo_warnings(false)` on the `cc::Build` object in the Linux cfg block. This tells cc-rs to suppress cargo warning messages, but as a side effect it drops the compiler's stderr entirely. If the C++ compilation fails, you only see `exit status: 1` with no actual error message.

This has been removed from the Linux block. If you see a mysterious cc-rs failure with no error output, check whether `cargo_warnings(false)` has been re-added.
