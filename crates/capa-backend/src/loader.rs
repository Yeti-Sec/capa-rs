...
fn extract_strings(bytes: &[u8]) -> Vec<StringInfo> {
    ...
    strings.extend(extract_ascii_strings(bytes));

    // Extract wide strings (UTF-16LE)
    strings.extend(extract_wide_strings(bytes));

    strings
}

/// Extract ASCII printable strings
fn extract_ascii_strings(bytes: &[u8]) -> Vec<StringInfo> {
    let mut strings = Vec::new();
...
/// Extract wide strings (UTF-16LE) commonly used in Windows malware
fn extract_wide_strings(bytes: &[u8]) -> Vec<StringInfo> {
    let mut strings = Vec::new();
    let mut current = String::new();
    let mut start_addr = 0u64;
    let mut i = 0;
    ...
    while i + 1 < bytes.len() {
        ...
        // Check for printable ASCII in UTF-16LE (hi byte should be 0)
        if hi == 0 && lo >= 0x20 && lo < 0x7F {
            ...
            current.clear();
            i += 1; // Move by 1 to handle misaligned strings
        }
    }
